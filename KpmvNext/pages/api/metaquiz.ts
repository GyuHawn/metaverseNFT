...
// insert
async function DbWrite(content:String, answer:String, explain:String){
...
//수정
async function DbUpdate1(content:String, answer:String, explain:String){
  const clc1 = await DbConnect1();
  const result = await clc1.updateOne({content:content},{$set:{answer, explain}});
}
//삭제
...
export default async (req: NextApiRequest, res: NextApiResponse)=>{
  ...
  if(read){
    ...
    await DbWrite(String(add), String(req.query.answer), String(req.query.explain));
    return res.send(await DbReadAll());
  }else if(update){
    await DbUpdate1(String(update), String(req.query.answer), String(req.query.explain));
    return res.send(await DbReadAll());
  }else if(del){
